...
set.seed(3)
dim <- length(coef(lm(log.model.noWH)))-1

regfit.full=regsubsets(log.model.noWH,df, nvmax = dim)
reg.summary=summary(regfit.full)

reg.summary$rsq # R2 statistic increases monotonically as more variables are included.
which.min(reg.summary$rss) # identify the location of the minimum in the model with 21 predictors. 
which.max(reg.summary$adjr2) # in fact Adjusted R2 statistic is max in the model with 18 predictors.
which.min(reg.summary$bic) # BIC is min in the model with 11 predictors
which.min(reg.summary$cp) # Cp is min in the model with 17 predictors
...
## Forward
regfit.fwd=regsubsets(log.model.noWH,data=df, nvmax=11, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is recommended.

## Backward
regfit.bwd=regsubsets(log.model.noWH,data=df,nvmax=11, method ="backward")
summary(regfit.bwd)

# The models found by best subset, forward and backward selection are equal.
ii=11; summary(regfit.full)$outmat[ii,]==summary(regfit.fwd)$outmat[ii,]
...
round(coef(regfit.full,11),3)
round(coef(regfit.fwd,11),3)
round(coef(regfit.bwd,11),3)
...
regfit.best=regsubsets(log.model.noWH,data=df[train,], nvmax = 11)
...
test.mat=model.matrix(log.model.noWH,data=df[test,])
...
# The best model is the one that contains which.min(val.errors) =  11 variables.
