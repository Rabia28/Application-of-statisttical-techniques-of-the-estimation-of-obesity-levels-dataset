...
pcr.fit=pcr(log.model.noWH.PCR,data = df ,scale=TRUE, validation ="CV")
...
set.seed (1)
x = model.matrix(log.model.noWH.PCR,data = df)[,-1]
...
pcr.fit=pcr(log.model.noWH.PCR,data = df ,subset=train,scale=TRUE,
            validation ="CV")
dev.new()
# Plot MSE and RMSE 
validationplot(pcr.fit,val.type="MSEP",legendpos = "topright") 
...
# Finally, we fit PCR on the full data set, using M = 11
...
# Setting scale=TRUE has the effect of standardizing each predictor (scale projection).
# Setting validation = 'CV' has the effect to use cross validation to rate M parameter
pls.fit=plsr(log.model.noWH.PCR,data = df, scale=TRUE, 
             validation ="CV")
summary(pls.fit)
dev.new()
...
set.seed (1)
pls.fit=plsr(log.model.noWH.PCR,data = df, subset=train, 
...
pls.fit=plsr(log.model.noWH.PCR,data = df ,scale=TRUE,ncomp=5)
summary(pls.fit)
