...
x = model.matrix(linear.model.noWH, df)[,-1] #[-1] means no intercept
...
dim(coef(ridge.mod)) # 22 coefficients, 100 lambda values
...
predict(ridge.mod,s=50,type="coefficients")[1:22,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:22,]
...
fit.lm=lm(linear.model.noWH,data=df)
coef(fit.lm)
#genderMale is the least influential
