...
##df <- df[,-1]
attach(df)
x = model.matrix(linear.model.drop, df)[,-1] #[-1] means no intercept
...
dim(coef(ridge.mod)) # 15 coefficients, 100 lambda values
...
predict(ridge.mod,s=50,type="coefficients")[1:15,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:15,]
...
fit.lm=lm(bmi~.-Weight-Height-SMOKEyes-drinks_alcoholno-drinks_alcoholSometimes
          -drinks_alcoholFrequently-counts_caloriesyes-num_meals-GenderMale,data=df)
coef(fit.lm)
