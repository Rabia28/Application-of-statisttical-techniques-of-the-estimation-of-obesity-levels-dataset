...
set.seed(3)
regfit.full=regsubsets(linear.model.all,df)
...
## Forward
regfit.fwd=regsubsets(linear.model.all,data=df, nvmax=8, method ="forward")
summary(regfit.fwd)
# We see that using forward stepwise selection, the best one-variable is recommended.

## Backward
regfit.bwd=regsubsets(linear.model.all,data=df,nvmax=8, method ="backward")
summary(regfit.bwd)
...
regfit.best=regsubsets(linear.model.all,data=df[train,], nvmax =8)
...
test.mat=model.matrix(linear.model.all,data=df[test,])
