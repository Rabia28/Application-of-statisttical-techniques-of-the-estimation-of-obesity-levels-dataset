...
x = model.matrix(linear.model.all, df)[,-1] #[-1] means no intercept
...
dim(coef(ridge.mod)) # 24 coefficients, 100 lambda values
...
predict(ridge.mod,s=50,type="coefficients")[1:24,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:24,]
...
# Figure that show the variation of the coefficients with different values of lambda
dev.new()
plot_glmnet(lasso.mod,label = T, xvar = "lambda")
...
# Compare with OLS when all predictors are included. 

fit.lm=lm(linear.model.all,data=df)
coef(fit.lm)
