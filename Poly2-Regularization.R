...
x = model.matrix(poly2.model.noWH, df)[,-1] #[-1] means no intercept

y = df$bmi

# The model.matrix() function is particularly useful for creating x; not only does it produce a matrix 
# corresponding to the 28 predictors but it also automatically transforms any qualitative
...
dim(coef(ridge.mod)) # 28 coefficients, 100 lambda values
...
predict(ridge.mod,s=50,type="coefficients")[1:28,] # s = lambda value, predict of the coefficients
...
lm(y~x, subset=train)
predict(ridge.mod,s=0,exact=T,type="coefficients",x=x[train,],y=y[train])[1:28,]
